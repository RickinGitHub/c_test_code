...
void *client_ctrl(void *arg)
{
    ...
    while (1) {
		if((recv_len = recv(sock_ctrl, recv_buf, 128, 0)) > 0) {
			...
			ctrl_cmd_handle(sock_ctrl, recv_buf, 128);
			memset(recv_buf, 0, size);
		}
    }
...
void ctrl_cmd_handle(int socket, char *buf, int len)
{
	...
	cmd = atoi(buf);
	switch(cmd) {
	case RMT_CALL_IN_REQ:
		send(socket, "RMT_CALL_IN_REQ", 16, 0);
		break;
    case RMT_HANDUP_SYNC:
		send(socket, "RMT_HANDUP_SYNC", 16, 0);
		break;
    case MSR_ANSWER_SYNC:
		send(socket, "MSR_ANSWER_SYNC", 16, 0);
		break;
    case MSR_HANDUP_SYNC:
		send(socket, "MSR_HANDUP_SYNC", 16, 0);
		break;
#if 1
	case SLV_ANSWER_SYNC:
		send(socket, "SLV_ANSWER_SYNC", 16, 0);
		break;
    case SLV_HANDUP_SYNC:
		send(socket, "SLV_HANDUP_SYNC", 16, 0);
		break;
#endif
	default:
		break;
