...
{     
    ...
    while(1) 
    {          
        if(!tcp_ctrl_events.server_flag)   
        {   
            while((tcp_ctrl_events.server_fd=socket(AF_INET,SOCK_STREAM,0))==-1)
            {
            ...
            bFlag = 1;
            if(setsockopt(tcp_ctrl_events.server_fd,SOL_SOCKET,SO_REUSEADDR,(char*)&bFlag,sizeof(bFlag)) == -1)//in binding, allow local addr reusing
            {
            ...
            tcp_server_addr.sin_port=htons(tcp_ctrl_events.port);

            while(bind(tcp_ctrl_events.server_fd,(struct sockaddr *)(&tcp_server_addr),sizeof(struct sockaddr))==-1)//judge rebinding or not
            {
                fprintf(stderr,"bind error:%s\n\a",strerror(errno));
                msleep(10);
            }
        }
        tcp_ctrl_events.server_flag = 1;

        if(listen(tcp_ctrl_events.server_fd,MSG_MAX)==-1)      //fail the client give error is 
        {
            fprintf(stderr,"listen error:%s\n\a",strerror(errno));
            if(tcp_ctrl_events.server_fd > 0)
                close(tcp_ctrl_events.server_fd);
            tcp_ctrl_events.server_fd = -1;
            exit(0);
        }
        sin_size=sizeof(struct sockaddr_in);
        printf("wait accept......\n");

        if( (client_fd=accept(tcp_ctrl_events.server_fd, (struct sockaddr *)(&tcp_client_addr), (socklen_t *)&sin_size )) == -1 )
        {
        ...
        if(tcp_ctrl_events.client_fd > 0)
        {
            ...
            close(tcp_ctrl_events.client_fd);
        }
        tcp_ctrl_events.client_fd = client_fd;
