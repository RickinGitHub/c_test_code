...
void *audio_send_handle(void *arg)
{
    ...
    {          
        if(!server_flag)   
        {   
            while((server_fd=socket(AF_INET,SOCK_STREAM,0))==-1)
            {
            ...
            bFlag = 1;
            if(setsockopt(server_fd,SOL_SOCKET,SO_REUSEADDR,(char*)&bFlag,sizeof(bFlag)) == -1)//in binding, allow local addr reusing
            {
                printf("%s : %s\n",__func__,strerror(errno));
            }
            ...
            while(bind(server_fd,(struct sockaddr *)(&tcp_server_addr),sizeof(struct sockaddr))==-1)//judge rebinding or not
            {
        ...
        if(listen(server_fd,MSG_MAX)==-1)      //fail the client give error is 
        {
        ...
        if( (client_fd=accept(server_fd, (struct sockaddr *)(&tcp_client_addr), (socklen_t *)&sin_size )) == -1 )
        {
...
void *server_send(void *arg)
{
    ...
    while (1)
    {
    	printf("phone stat:%d, micmute stat:%d\n",get_phone_state(), get_mic_state());
		if((get_phone_state() == PHONE_STATE_ACTIVATED) && (get_mic_state() == ENABLE)) {
			pthread_mutex_lock(&mutex_audio);
		    rc = snd_pcm_readi(pcm_capture_handle, send_buf, frames);
	        if (rc == -EPIPE) {
	            /* EPIPE means overrun */
...
void *server_recv(void *arg)
{
	...
	while(1) {
		while ((recv_len = recv(client_fd, recv_buf, size, 0)) > 0) {
			...
			if((get_phone_state() == PHONE_STATE_ACTIVATED) && (get_mic_state() == ENABLE)) {
				pthread_mutex_lock(&mutex_audio);
				rc = snd_pcm_writei(pcm_playback_handle, recv_buf, frames);
				if (rc == -EPIPE) {
