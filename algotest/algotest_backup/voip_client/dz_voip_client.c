...
int main(int argc, char** argv)
{
	...
	while(1) {
		if(!client_status) {
			//创建接收线程
			pthread_create(&thread_id_recv, NULL, &client_recv, &argv[1]);
			pthread_detach(thread_id_recv);

			//创建发送线程
			pthread_create(&thread_id_send, NULL, &client_send, &argv[1]);
...
snd_pcm_uframes_t dz_set_params(snd_pcm_t **handle, int direct, int rate)
{
	...
	/* Two channels (stereo) */
	if(direct == PLAYBACK)
		rc=snd_pcm_hw_params_set_channels(phandle, params, DEF_PLAYBACK_CHANALS);
	if(direct == CAPTURE)
		rc=snd_pcm_hw_params_set_channels(phandle, params, DEF_CAPTURE_CHANALS);
	if(rc<0)
	{
...
void *client_recv(void *arg)
{
	...
	while(1) {
		if((recv_len = recv(sock_recv, recv_buf, size, 0)) > 0)
    	{
			...
			fwrite((char *)recv_buf, 1, recv_len, fp);
...
void *client_send(void *arg)
{
	...
	fp = fopen("clien_send_dump", "a+");
	if (fp ==NULL) {
		printf("open clien_send_dump failed\n");
	}
    ...
    while (1)
    {
		...
		fwrite((char *)send_buf, 1, size, fp);
