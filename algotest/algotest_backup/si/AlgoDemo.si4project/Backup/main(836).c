...
#ifdef ALGORITHM	
	output_bf_audio = fopen("server_dump.pcm", "a+");
	if (output_bf_audio ==NULL) {
		printf("open file failed\n");
	}else {
		printf("open output_bf_audio ok\n");
	}

	readi_buff = fopen("readi_dump.pcm", "a+");
	if (readi_buff ==NULL) {
		printf("open /tmp/readi_dump.pcm failed\n");
	}else {
		printf("open readi_buff ok\n");
	}
